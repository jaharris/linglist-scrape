...
Use the code and as you like, but please just give a [shout out](http://linguistics.ucla.edu/person/jesse-harris/) if you use this script.  **Comments are most welcome!**
...
Enter dates to search here. Set to search Linguist List from `r  start` to `r end`.
...
if (no.applied == TRUE) {
df <- subset(df, !str_detect(tolower(df$keywords), paste(c('applied', 'applied', 'anthro',  'l2 acquisition', 'language teaching', 'teaching methodology'), collapse = '|')))
}
...
# Add year column
tt.year.norm$year <- as.numeric(rownames(tt.year.norm))
...
colnames(df.tt.norm) <- c('year', 'field', 'jobs.norm')
